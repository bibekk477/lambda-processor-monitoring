...
resource "aws_cloudwatch_metric_alarm" "lambda_errors" {
  ...
  alarm_actions       = [var.critical_alerts_topic_arn] #send a notification to this SNS topic whenever the alarm goes into ALARM state.
  ok_actions          = [var.critical_alerts_topic_arn]

  dimensions = { # Filters metric specifically for this Lambda function.
...
resource "aws_cloudwatch_metric_alarm" "low_success_rate" {
  ...
  actions_enabled     = true
  alarm_actions       = [var.performance_alerts_topic_arn]
  treat_missing_data  = "notBreaching" # f metric data is missing for a period, treat it as if the threshold was exceeded, so the alarm goes into ALARM state
